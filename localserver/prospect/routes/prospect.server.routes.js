...
router.post('/fetch/defaultparam', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call get_default_param_V2(?, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/fetch/growthparam', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call Get_Growth_Param_v4(?, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/save/growthparam', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call save_growth_param_v2(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/fetch/ibqtytcv', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call GET_IB_QTY_TCV_v6(?, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/fetch/manualqtytcv', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call MANUAL_QTY_TCV_v5(?, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/save/manualqtytcv', (req, res) => {
  ...
  recordsArr.forEach(function(recordObj) {
    ...
    paramArr.push(recordObj.prospectrefid, recordObj.suite, recordObj.lineItemId,
      ...
      recordObj.createdDate, recordObj.createdBy, recordObj.manualSubQty);
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call SAVE_Manual_QTY_v1(TEMP_EA_PROSPECT_REF_QTY_UPDATES_v1)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/fetch/report', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call Generate_Report_v1(?, ?, ?, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
      ...
      return statement.exec([prospectrefid, cecUser, createdBy, updatedBy], (err, parameters, rows, tableRows) => {
        //hanaClient.disconnect();
        if (err) {
          console.error('Execute error:', err);
          return res.jsonp({
...
router.post('/save/tcodata', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call ea_persist_tco_data(?)_V1', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
router.post('/upload/euif', (req, res) => {
  ...
  if (hanaClient.readyState === 'connected') {
    return hanaClient.prepare('call ea_save_euif_form_pr_v1(?, TEMP_EA_EUIF_FORM_TAB, ?)', (err, statement) => {
      if (err) {
        //hanaClient.disconnect();
        console.error('Prepare error:' + err);
        return res.jsonp({
...
function updateProspectRefTemporaryTable(values, cb) {
  function truncate(cb) {
    var sql = 'truncate table TEMP_EA_PROSPECT_REF_QTY_UPDATES_V1';
  ...
  function prepare(cb) {
    var sql = 'insert into TEMP_EA_PROSPECT_REF_QTY_UPDATES_V1 values(?, ?, ?, ?, ?, ?, ?, ?, ?)';
    hanaClient.prepare(sql, cb);
  }

  function insertValues(statement, cb) {
    function createTasks(value) {
      return statement.exec.bind(statement, [value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7], value[8]]);
    }
